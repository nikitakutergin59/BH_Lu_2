...
func CalculateHandler(w http.ResponseWriter, r *http.Request, md *MemoryData, calcContext *CalculateContext) {
	...
	tokens_BH, err := tokens.Tokenize_BH(req.Expression)
	...
	log.Printf("CalculateHandler: токены: %+v", tokens_BH) // 16
	...
	go func() {
		...
		newTokensPE, err := processExpression(expr, tokens_BH, calcContext)
		if err != nil {
			...
			log.Println("CalculatorHandle: newTokenPE", newTokensPE)
			errChan <- nil
		}
	}()
...
func processExpression(expr *Expression_BH, tokens_BH []tokens.Token, calcContext *CalculateContext) ([]tokens.Token, error) {
	log.Println("processExpression: начало") // 1
	var (
		stack       [][]tokens.Token
		newTokens   []tokens.Token
		newTokensPE []tokens.Token // Переменные, которые будут аккумулировать результаты
	)

	log.Println("expr", expr)
	log.Println("token_BH", tokens_BH)
	log.Println("calcCotext", calcContext)

	for i, token := range tokens_BH {
		if token.Type == tokens.TOKEN_PARENT_OPEN {
			stack = append(stack, []tokens.Token{})
			log.Printf("processExpression: i=%d stack=%+v", i, stack)
		} else if token.Type == tokens.TOKEN_PARENT_CLOSE {
			if len(stack) == 0 {
				return nil, fmt.Errorf("некорректные скобки в выражении")
			}

			lastExpr := stack[len(stack)-1]
			log.Println("processExpression: lasrExpr:", lastExpr)
			stack = stack[:len(stack)-1]

			if len(lastExpr) < 1 {
				return nil, fmt.Errorf("некорректное выражение внутри скобок: %v", lastExpr)
			}
			...
			newTokensPE, err = processExpression(subExpr, lastExpr, calcContext) // исправил, теперь правильно сохраняем значения
			if err != nil {
				costil := newTokensPE
				log.Println("newTokensPE", costil)
				return nil, err
			}
			...
			log.Println("processExpression: sudTask:", subTask)
			...
			newTokensPE = append(newTokens, tokens.Token{
	...
	log.Println("processExpression: завершение")
	log.Println("processiExpression: вернула", newTokensPE, nil)
	return newTokensPE, nil
}

func splitExpression(expr *Expression_BH, tokens []tokens.Token, calcContext *CalculateContext) error {
	log.Printf("splitExpression: expr.ID: %s", expr.ID)
	log.Printf("splitExpression: len(tokens): %d", len(tokens))
	log.Printf("splitExpression: tokens: %+v", tokens)

	if len(tokens) < 1 {
		err := fmt.Errorf("недостаточно токенов для выражения: %v", tokens)
		log.Printf("splitExpression: error: %v", err)
		return err
	}
	...
	for i := 0; i < len(tokens)-3; i += 2 {
		...
		log.Printf("splitExpression: iteration: %d, arg1: %s, operator: %s, arg2: %s", i/2, arg1, operator, arg2)

		taskID, err := generateID()
		if err != nil {
			err = fmt.Errorf("ошибка генерации ID: %w", err)
			log.Printf("splitExpression: error generating ID: %v", err)
			return err
		}

		log.Printf("splitExpression: taskID generated: %s", taskID)

		operatorTime, err := calcContext.GetOperatorTime(operator)
		if err != nil {
			err = fmt.Errorf("ошибка получения времени оператора %w", err)
			log.Printf("splitExpression: error getting operator time: %v", err)
			return err
		}
		log.Printf("splitExpression: operatorTime: %d", operatorTime)
		...
		log.Println("splitExpression: task:", task)

		// Добавляем задачу в контекст
		calcContext.Md.mu.Lock()
		log.Println("splitExpression: Mutex locked")
		calcContext.Md.Task[taskID] = task
		log.Printf("splitExpression: Task added to calcContext.Md.Task[%s]", taskID)
		expr.Tasks = append(expr.Tasks, task)
		log.Printf("splitExpression: Task added to expr.Tasks, len(expr.Tasks): %d", len(expr.Tasks))
		calcContext.Md.mu.Unlock()
		log.Println("splitExpression: Mutex unlocked")
	}

	log.Printf("splitExpression: returning nil error")
	return nil
}
...
func ReseiveTaskResultHandler(w http.ResponseWriter, r *http.Request, md *MemoryData) {
	if r.Method != http.MethodPost {
